...
import { GstandaardSearchService } from '../gstandaard-search.service';

const SEARCH_DEBOUNCE_MS = 300;
const MIN_SEARCH_LENGTH = 2;

@Component({
  selector: 'app-gstandaard-contraindication-modal',
  imports: [Modal, LucideCircleAlert, LucideSearch, LucideX],
  template: `
    <app-modal [open]="open()" title="G-Standaard contraindicaties toevoegen" (close)="close.emit()">
      <svg modalIcon lucideCircleAlert [size]="20"></svg>

      <label class="mb-3 block">
        <span class="sr-only">G-Standaard contraindicaties zoeken</span>
        <div class="relative">
          <svg
            lucideSearch
            [size]="16"
...
export class GstandaardContraindicationModal {
  private readonly gstandaardSearch = inject(GstandaardSearchService);
  private readonly store = inject(GstandaardContraindicationStore);
  ...
  protected readonly searchResource = resource({
    ...
    loader: ({ params: term }) => this.gstandaardSearch.search(term),
  });
  ...
  protected toggle(item: GstandaardContraindication) {
