  ...
  template: `
    <app-modal [open]="open()" title="Patiënt selecteren" (close)="close.emit()">
      ...
      @if (patientStore.actionError(); as error) {
        <div class="mb-3 rounded-md border border-red-300 bg-red-50 px-3 py-2 text-sm text-red-800" role="alert">
          {{ error }}
        </div>
      }
      ...
      @if (patientStore.loadError(); as error) {
        <div class="mb-3 flex items-center justify-between rounded-md border border-red-300 bg-red-50 px-3 py-2 text-sm text-red-800" role="alert">
          <span>{{ error }}</span>
          <button
            type="button"
            class="rounded border border-red-300 px-2 py-1 text-xs font-medium hover:bg-red-100 focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-600"
        ...
        <table class="w-full text-left text-sm">
          <thead class="tracking-wide border-b border-gray-200 text-gray-500">
            <tr>
              ...
              <th class="w-28 px-3 py-2"><span class="sr-only">Acties</span></th>
            </tr>
          </thead>
          <tbody
            cdkDropList
              ...
              >
                ...
                <td class="px-3 py-2">
                  <div class="flex items-center justify-end gap-1">
                    <button
                      type="button"
                      class="rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-700 disabled:pointer-events-none disabled:opacity-30 focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-600"
                      [disabled]="isSearching() || i === 0"
                      [attr.aria-label]="patient.name + ' omhoog verplaatsen'"
                      (click)="move(patient.id, 'up', $event)"
                    >
                    ...
                    <button
                      type="button"
                      class="rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-700 disabled:pointer-events-none disabled:opacity-30 focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-600"
                      [disabled]="isSearching() || i === filteredPatients().length - 1"
                      [attr.aria-label]="patient.name + ' omlaag verplaatsen'"
                      (click)="move(patient.id, 'down', $event)"
                    >
                      <svg lucideChevronDown [size]="16"></svg>
                    </button>
                    <button
                      type="button"
                      class="rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-700 focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-600"
                    ...
                    <button
                      type="button"
                      class="rounded p-1 text-gray-400 hover:bg-red-50 hover:text-red-600 focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-600"
...
export class PatientSelectModal {
  ...
  protected move(id: string, direction: 'up' | 'down', event: Event) {
    event.stopPropagation();
    this.patientStore.movePatient(id, direction);
  }
