...
import { IcpcContraindicationApiService } from './icpc-contraindication-api.service';
import { IcpcContraindication } from './icpc.model';

@Service()
export class IcpcContraindicationStore {
  private readonly api = inject(IcpcContraindicationApiService);
  private readonly patientStore = inject(PatientStore);

  private readonly contraindicationsState = signal<IcpcContraindication[]>([]);
  ...
  async add(item: IcpcContraindication) {
